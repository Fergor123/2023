...
while robot.step(timestep) != -1:
    ...
    print(motion_goal_flag)
   
    if(motion_goal_flag):
        motion_goal_flag = abs(full_range_image[0] + 0.04) > 0.16 or obj_in_view
        if(not idle_flag):
            if(not_search_flag):
                setMotion(3.14, 3.14)
                print("Approaching")
        ...
        if(obj_in_view and (search_centered_flag and pos_view - full_range_image[0]  < .56)):
            motion_goal_flag= True
        front_wall = front_dist < .05
        if front_wall:
            rotation_flag = True
        if not rotation_flag:
            correctionPID(wall_anchor, wall_target, min(full_range_image[255:285]), k_ps, max_vel)
        else:
            if (right_dist > left_dist or right_bias):
                right_bias = True
                setMotion(1.5, -1.5)
            else:
                setMotion(-1.5, 1.5)
